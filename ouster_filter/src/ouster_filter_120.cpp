...
//This class takes a PointCloud2 as an input and republishes only the point in a 120 degres field of view,
//The robot is also filtered out
class OusterFilter {
    protected:
    ...
    protected:
        ...
        void pc_callback(const sensor_msgs::PointCloud2ConstPtr msg) {
            ...
            for (auto it = cloud.points.begin(); it != cloud.points.end(); ++it){
                ...
                //If the point is not in the fov, we ignore:
                double theta = atan2(it->y, it->x);
                if (fabs(theta) > (fov_/2)*M_PI/180) {
                    continue;
                }
